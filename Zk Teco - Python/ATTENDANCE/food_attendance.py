...
def get_machine_ip():
	with open("D:\\ATTENDANCE\\food_parameter\\ip_address.txt") as f:
		ip = f.readlines()
	ip = [x.strip() for x in ip] 
	return ip
	
def get_time_list():
	with open("D:\\ATTENDANCE\\food_parameter\\iteration_time_duration.txt") as f:
		list_time = f.readlines()
	list_time = [x.strip() for x in list_time]
	return list_time

def get_connection_info():
	with open("D:\\ATTENDANCE\\food_parameter\\db_connection.txt") as f:
